      ...
      <AnimatePresence>
        {isOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            {/* Modal Content */}
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative w-full max-w-4xl overflow-hidden rounded-3xl bg-[#1a1a1a] shadow-2xl border border-white/10" >
              {/* Premium Header */}
              <div className="flex items-center justify-between px-6 py-4 bg-[#1a1a1a]">
                ...
                <button onClick={closeModal} className="rounded-full bg-white/5 p-2 text-white/60 transition-all hover:bg-white/10 hover:text-white focus:outline-none" >
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="relative group">
                    ...
                    {videos.map((video, index) => (
                      <div className="flex-[0_0_100%] min-w-0" key={video.id}>
                        <div className="aspect-video w-full bg-black relative">
                          {selectedIndex === index ? (
                            <iframe width="100%" height="100%" src={`https://www.youtube.com/embed/${getYoutubeId(video.youtubeUrl)}?autoplay=1&rel=0&modestbranding=1&showinfo=0`} title={video.title} frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen className="h-full w-full" />
                          ) : (
                ...
                {/* Navigation Arrows */}
                <button onClick={scrollPrev} className="absolute left-4 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full bg-black/40 text-white/70 backdrop-blur-md flex items-center justify-center hover:bg-black/60 hover:text-white transition-all opacity-0 group-hover:opacity-100 focus:outline-none" >
                  <ChevronLeft className="h-6 w-6" />
                </button>
                <button onClick={scrollNext} className="absolute right-4 top-1/2 -translate-y-1/2 h-10 w-10 rounded-full bg-black/40 text-white/70 backdrop-blur-md flex items-center justify-center hover:bg-black/60 hover:text-white transition-all opacity-0 group-hover:opacity-100 focus:outline-none"  >
              ...
              <div className="h-14 bg-[#1a1a1a] flex items-center justify-center gap-6">
                ...
                <div className="flex gap-2">
                  {videos.map((_, index) => (
                    <button key={index} onClick={() => emblaApi && emblaApi.scrollTo(index)} className={`h-1.5 rounded-full transition-all duration-300 ${selectedIndex === index ? "w-8 bg-primary" : "w-1.5 bg-white/10"}`} />
                  ))}
                </div>
