...
// ─── Sous-composants réutilisés depuis la Boutique classique (aucune duplication de logique) ──
import ProductsManagementContent from "@/components/store/sections/components/ProductsManagementContent"
import StorePerformance from "@/components/store/sections/components/StorePerformance"
import SharePanel from "@/components/shared/SharePanel"
import RecentOrders from "@/components/store/sections/components/RecentOrders"

const ITEMS_PER_PAGE = 10

interface FournisseurBoutiqueProps {
    /** Permet de naviguer vers l'onglet "Commandes" du dashboard (optionnel — reçu depuis akwaba/page.tsx) */
    onNavigateToOrders?: () => void
}

/**
 * Espace "Produits" dédié au rôle FOURNISSEUR — composant volontairement séparé de
 * <Store /> (Boutique classique) : mêmes briques réutilisées (stats, catalogue, boost,
 * commandes, partage de lien), mais SANS le QR code de boutique, propre au B2C.
 */
export default function FournisseurBoutique({ onNavigateToOrders }: FournisseurBoutiqueProps) {
    ...
    const handleUpdateStore = async () => {
        if (!newStoreName.trim()) {
            addNotification("Le nom de votre entreprise est requis", "error")
            return
        }
        ...
        try {
            ...
            if (res.statusCode === 200 || res.statusCode === 201) {
                addNotification("Vitrine mise à jour avec succès", "success")
    ...
    const handleFormSubmit = async (formData: FormData) => {
        ...
        try {
            ...
            console.error("Error submitting supplier product:", error)
    ...
    const handleToggleStatus = async (product: Product, value: boolean) => {
        try {
            ...
            console.error("Error toggling supplier product status:", error)
    ...
    const fournisseurUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/fournisseur/${createStoreSlug(storeInfo?.storeName || '')}`

    return (
        <div className="flex flex-col items-center w-full max-w-7xl mx-auto px-0 md:px-4 py-2">
            <SectionHeader title="Votre catalogue B2B, géré en toute simplicité"
                subtitle="Publiez vos produits en gros et demi-gros, suivez vos demandes de devis et vos commandes depuis un espace dédié aux fournisseurs et grossistes."
                className="mb-6"
            />

            {/* ── Hero : identité fournisseur ── */}
                ...
                <div className="group bg-card border border-border rounded-3xl p-5 md:p-6 flex items-center justify-between gap-6 shadow-sm">
                    ...
                    <div className="flex items-center gap-4 md:gap-6 min-w-0">
                        <div className="relative w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center border border-primary/10 overflow-hidden shrink-0">
                            {storeInfo?.storeLogo ? (
                                <Image src={storeInfo.storeLogo} alt={storeInfo.storeName || "Fournisseur"} fill className="object-cover" unoptimized />
                            ) : (
                                <Icon icon="mdi:warehouse" className="w-8 h-8 md:w-10 md:h-10 text-primary" />
                            )}
                        </div>

                        <div className="space-y-1 min-w-0">
                            <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-1">Ma Vitrine Fournisseur</p>
                            <h3 className="text-xl md:text-2xl font-black text-foreground uppercase tracking-tight leading-none truncate max-w-[150px] md:max-w-md">
                                {storeInfo?.storeName || "Mon Entreprise"}
                            </h3>
            ...
            <div className="flex flex-col md:flex-row w-full max-w-4xl mx-auto px-2 md:px-4 gap-2 md:gap-4 mb-8">
                <div className="w-full">
                    <CreateButton label="Publier un produit" loading={checkLoading} onClick={openCreateModal} />
                </div>
            ...
            {/* ── Actions rapides — sans QR code, propre au B2C ── */}
            <div className="w-full max-w-4xl mx-auto px-2 md:px-4 mb-8">
                <h3 className="text-lg font-black text-foreground mb-4">Développez votre activité B2B</h3>

                <div className="flex flex-col gap-3">

                    <AccordionSection id="catalogue" title="Catalogue" subtitle="Gérez vos produits en gros et demi-gros" icon="solar:widget-2-bold-duotone" activeSection={activeQuickAction} onToggle={toggleQuickAction} >
                    ...
                    <AccordionSection id="boost" title="Booster mes produits" subtitle="Gagnez en visibilité auprès des clients B2B" icon="solar:rocket-bold-duotone" activeSection={activeQuickAction} onToggle={toggleQuickAction} >
                    ...
                    <AccordionSection id="partager" title="Partager mon catalogue" subtitle="Invitez de nouveaux clients B2B" icon="solar:share-bold-duotone" activeSection={activeQuickAction} onToggle={toggleQuickAction} >
                        <SharePanel url={fournisseurUrl} label={storeInfo?.storeName || "Mon Entreprise"} />
                    </AccordionSection>
                ...
                <div className="p-4 md:p-0">
                    <div className="mb-6 md:mb-8 md:p-6 md:pb-0">
                        ...
                        <p className="text-sm text-muted-foreground mt-1 ml-14">
                            {isEditing ? 'Mettez à jour les informations de votre article' : 'Remplissez les détails pour ajouter un nouvel article à votre catalogue'}
                        </p>
                    </div>
                ...
                <div className="p-6 space-y-6">
                    <div className="flex flex-col items-center gap-4">
                        <div className="relative group">
                            ...
                            <p className="text-[10px] font-black text-center text-muted-foreground uppercase tracking-widest mt-2">Logo de l&apos;entreprise</p>
                        </div>
                        ...
                        <div className="w-full space-y-4">
                            <div className="space-y-1.5">
                                ...
                                <div className="relative">
                                    <Icon icon="mdi:warehouse" className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-primary" />
                                    <input type="text" placeholder="Ex: Ma Société d'Import-Export" className="w-full bg-muted/50 border border-border rounded-2xl py-4 pl-12 pr-4 text-sm font-black focus:border-primary outline-none transition-all" value={newStoreName} onChange={(e) => setNewStoreName(e.target.value)} />
                                </div>
                            </div>
                        </div>
                    </div>
