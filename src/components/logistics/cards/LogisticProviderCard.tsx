            ...
            <div onClick={handleNavigate}
                ...
                <div className={`flex flex-col flex-1 min-w-0 ${viewMode === 'grid' ? "px-0.5 pb-0 md:px-0 md:pb-0 w-full" : "h-full justify-center"}`}>
                    ...
                    <div className={`${viewMode === 'grid' ? "pt-3 w-full" : "mt-2 md:mt-4"}`}>
                        <button className={`flex items-center gap-1 md:gap-2 bg-secondary text-white rounded-full font-black hover:bg-primary transition-all active:scale-90 shadow-sm ${viewMode === 'grid' ? "px-2 py-1 md:px-3 md:py-2 text-[10px] md:text-xs mx-auto" : "px-3 py-1.5 md:px-5 md:py-2.5 text-xs md:text-sm mr-auto"
                            }`}>
                            Voir les services
