                ...
                {loading && providers.length === 0 ? (
                    ...
                    <InfiniteScroll
                        ...
                        renderItem={(provider: GasProvider) => (
                            <div key={provider.id} onClick={() => withAuth(() => setSelectedProvider(provider))} className={`group rounded-xl transition-all duration-300 cursor-pointer bg-card border border-border/40 hover:border-primary/30 overflow-hidden ${viewMode === 'grid' ? "p-3 md:p-4 flex flex-col items-center text-center" : "p-3 md:p-4 flex flex-row items-center gap-4 text-left"}`}>
                                {/* Icône bouteille de gaz */}
                                <div className={`relative shrink-0 rounded-2xl bg-primary/10 flex items-center justify-center ${viewMode === 'grid' ? "w-16 h-16 mb-3" : "w-16 h-16 md:w-20 md:h-20"}`}>
