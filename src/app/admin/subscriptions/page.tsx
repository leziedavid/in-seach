...
export default function AdminSubscriptionsPage() {
    ...
    return (
        <>
            <div className="p-3 sm:p-4 md:p-6 space-y-4 md:space-y-5 animate-in fade-in duration-500">
            <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="min-w-0">
                    <h1 className="text-xl sm:text-2xl font-bold tracking-tight">{t("admin.subscriptions.title")}</h1>
                    <p className="text-muted-foreground text-xs sm:text-sm mt-0.5">{t("admin.subscriptions.subtitle")}</p>
                </div>
                <div className="overflow-x-auto flex-shrink-0">
                    <div className="flex items-center gap-1 p-1 bg-muted rounded-xl w-max">
                        <button
                            onClick={() => setActiveTab('overview')}
                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all whitespace-nowrap ${activeTab === 'overview' ? 'bg-card shadow-xs text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
                        >
                            {t("admin.subscriptions.tabs.overview")}
                        </button>
                        <button
                            onClick={() => setActiveTab('plans')}
                            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all whitespace-nowrap ${activeTab === 'plans' ? 'bg-card shadow-xs text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
                        >
            ...
            {activeTab === 'overview' && (
                <div className="space-y-4 md:space-y-5 animate-in cubic-bezier duration-500">
                    <Card className="rounded-2xl border-border/50 shadow-xs overflow-hidden">
                        <CardContent className="p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            <div className="flex items-center gap-3">
                                <div className={`p-2.5 rounded-xl border transition-all flex-shrink-0 ${isSystemEnabled ? 'bg-primary/10 border-primary/20 text-primary' : 'bg-muted border-border text-muted-foreground'}`}>
                                    <Settings2 className="w-5 h-5" />
                                </div>
                                <div className="min-w-0">
                                    <CardTitle className="text-sm font-semibold">{t("admin.subscriptions.system.title")}</CardTitle>
                                    <CardDescription className="text-xs mt-0.5">{t("admin.subscriptions.system.desc")}</CardDescription>
                                </div>
                            </div>
                            <div className="flex items-center gap-3 flex-shrink-0">
                                <Badge variant={isSystemEnabled ? "default" : "secondary"} className="font-semibold uppercase tracking-wide text-[10px]">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                        {[
                            ...
                            <div key={i} className="bg-white dark:bg-zinc-900 rounded-2xl p-3 sm:p-4 border border-border shadow-sm flex items-center gap-3 min-w-0 overflow-hidden">
                                <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: `${stat.color}18` }}>
                                    <stat.icon className="w-4 h-4" style={{ color: stat.color }} />
                                </div>
                                <div className="min-w-0 flex-1">
                                    <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wider truncate">{stat.label}</p>
                                    <p className="text-lg font-bold text-foreground tabular-nums truncate mt-0.5">{stat.val}</p>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            {activeTab === 'plans' && (
                <div className="space-y-4 md:space-y-5 animate-in cubic-bezier duration-500">
                    <div className="flex justify-end">
                        <Button onClick={handleCreatePlan} className="rounded-xl font-semibold gap-2 text-xs h-8 px-3">
                            <Plus className="w-3.5 h-3.5" /> {t("admin.subscriptions.plans.new")}
                        </Button>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {plans.map((plan) => (
                            <div key={plan.id} className={`rounded-2xl border border-border/50 shadow-sm overflow-hidden group transition-all flex flex-col ${!plan.isActive ? 'bg-muted/30 grayscale-[0.5] opacity-80' : 'bg-card hover:border-primary/50 hover:shadow-md'}`}>
                                <CardHeader className="p-4 pb-3">
                                    <div className="flex items-center justify-between mb-3">
                                        <div className="flex items-center gap-2">
                                            <Badge variant={plan.isActive ? "default" : "secondary"} className="font-semibold text-[9px] uppercase tracking-wide px-2 py-0.5">
                                        ...
                                        <div className="flex gap-1">
                                            <Button variant="ghost" size="icon" onClick={() => handleEditPlan(plan)} className="h-7 w-7 rounded-lg"><Edit3 className="w-3 h-3 text-muted-foreground" /></Button>
                                            <Button variant="ghost" size="icon" onClick={() => handleDeletePlan(plan.id)} className="h-7 w-7 rounded-lg hover:text-rose-500"><Trash2 className="w-3 h-3 text-muted-foreground" /></Button>
                                        </div>
                                    </div>
                                    <CardTitle className="text-lg font-bold">{plan.name}</CardTitle>
                                    <div className="flex items-baseline gap-1 pt-1">
                                        <span className="text-2xl font-bold text-primary">{plan.price} CFA</span>
                                        <span className="text-muted-foreground font-medium text-xs">/{t("admin.subscriptions.plans.period", { days: plan.durationDays })}</span>
                                    </div>
                                </CardHeader>
                                ...
                                <div className="px-4 py-3 border-t border-border/30 bg-muted/20 flex items-center">
            ...
            {activeTab === 'users' && (
                <div className="space-y-4 md:space-y-5 animate-in cubic-bezier duration-500">
                    <div className="flex justify-between items-center gap-3">
                        <div className="min-w-0">
                            <h2 className="text-base font-semibold">{t("admin.subscriptions.users.title")}</h2>
                            <p className="text-muted-foreground text-xs mt-0.5">{t("admin.subscriptions.users.subtitle")}</p>
                        </div>
                        <Button onClick={() => setIsAssignModalOpen(true)} className="rounded-xl font-semibold gap-2 text-xs h-8 px-3 flex-shrink-0 bg-primary hover:bg-primary/90">
                            <ShieldCheck className="w-3.5 h-3.5" /> {t("admin.subscriptions.users.assign_manual")}
                        </Button>
                    </div>

                    <div className="bg-card rounded-2xl border border-border/50 shadow-xs overflow-x-auto p-2">
