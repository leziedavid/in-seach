    ...
    return (
        <div className="min-h-screen bg-muted/30 text-foreground flex overflow-hidden">
            ...
            {/* ══════════════════════════════════════════════════
                SIDEBAR
            ══════════════════════════════════════════════════ */}
            <aside className={` ${isSidebarOpen ? 'w-[235px]' : 'w-[72px]'} bg-card border-r border-border h-screen sticky top-0 transition-all duration-300 ease-in-out flex flex-col z-50 flex-shrink-0 shadow-sm`}  >
                {/* ── Logo ──────────────────────────────────── */}
                ...
                <nav className="flex-1 px-3 mt-1 space-y-0.5 overflow-y-auto overflow-x-hidden scrollbar-hide">
                    {menuItems.map((item) => {
                        ...
                        return (
                            <Link key={item.href} href={item.href} title={!isSidebarOpen ? item.label : undefined} className={`relative flex items-center rounded-2xl font-semibold transition-all duration-200 group ${isSidebarOpen ? 'gap-3 px-3.5 py-2.5' : 'justify-center py-2.5'} ${isActive ? 'bg-primary text-secondary-foreground shadow-lg shadow-primary/20' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}>
                                {/* Icône Solar Bold Duotone */}
                                <Icon icon={item.icon} width={isSidebarOpen ? 20 : 22} className={`flex-shrink-0 transition-all ${isActive ? 'text-secondary-foreground' : ''}`} />
                ...
                <div className="mt-auto">
                    ...
                    <div className="px-3 mb-2">
                        <Link href="/" title={!isSidebarOpen ? "Quitter l'admin" : undefined}
                            className={`flex items-center rounded-2xl py-2.5  text-muted-foreground hover:text-rose-500 hover:bg-rose-500/8 transition-all group ${isSidebarOpen ? 'gap-3 px-3.5' : 'justify-center'}`}>
                            <Icon icon="solar:logout-bold-duotone" width={20} className="flex-shrink-0" />
                            {isSidebarOpen && (
                    ...
                    {/* User card */}
                    <div className={`mx-3 mb-5 flex items-center rounded-2xl bg-muted px-3 py-3 gap-3 overflow-hidden ${!isSidebarOpen ? 'justify-center' : ''}`}>
                        ...
                        {isSidebarOpen && (
                            <>
                                ...
                                <Icon icon="solar:alt-arrow-right-bold-duotone" width={16} className="text-muted-foreground flex-shrink-0" />
                            </>
                        )}
                    </div>
                </div>
